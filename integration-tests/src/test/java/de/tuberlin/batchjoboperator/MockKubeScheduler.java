...
import static de.tuberlin.batchjoboperator.common.util.General.logAndRethrow;
...
class MockKubeScheduler {
                ...
                nodeSelector.stream()
                            .anyMatch(ns -> ns.getMatchExpressions().stream()
                                              ...
                                              .allMatch((expression) -> {
                                                  ...
                                                  assertThat(expression.getOperator())
                                                          .as("Only In Operator is supported")
                                                          .isEqualToIgnoringCase("in");

                                                  var nodeLabels =
                                                          getNullSafe(() -> nodes.get(nodeName)
                                                                                 .getMetadata()
                                                                                 .getLabels())
                                                                  .orElse(emptyMap());
    ...
    private synchronized void filterNodes(Pod resource, int retries) {
        final var pod = client.pods().inNamespace(getNamespace(resource)).withName(getName(resource)).get();
        ...
        if (possibleNodes.size() > 0) {
            ...
            var result = logAndRethrow(() -> uut.filter(filterArgs));
            log.info("Filter result: {}", result);

            var targetNodeName = requireFirstElement(result.getNodeNames());
        ...
        var result = logAndRethrow(() -> uut.preemption(preemptionArgs));

        log.info("preemption result: {}", result);
        var victimPodUid = requireFirstElement(result.getNodeNameToMetaVictims().entrySet())
    ...
    private void run() {
        var podWatch = client.pods().inAnyNamespace().watch(new Watcher<Pod>() {
            ...
            public void eventReceived(Action action, Pod resource) {
                ...
                log.info("Pod {} requires scheduling", getName(resource));
