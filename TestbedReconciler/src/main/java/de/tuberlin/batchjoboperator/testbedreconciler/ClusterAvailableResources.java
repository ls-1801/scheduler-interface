...
public class ClusterAvailableResources {
    ...
    public static ClusterAvailableResources diff(ClusterAllocatableResources allocatableResources,
        ...
        if (!allocatableResources.allocatableResourcesMap.keySet().containsAll(
                requestResources.requestedResourceMap.keySet()
        )) {
            throw new RuntimeException("Allocatable resources does not include all nodes inside the requested " +
                    "resources");
        }
    ...
    public BigDecimal getAvailableResources(@Nonnull Node node, @Nonnull String resourceName) {
        ...
        if (nodesRequestedResources == null) {
            throw new RuntimeException(
                    MessageFormat.format("Node {0} is not part of the RequestedResourceMap", nodeName)
            );
        }
